...
export default class Line extends Coordinate {
  ...
  renderLabels(offset) {
    ...
    // 如果 xAxis align center，则 pointCount 无需减 1
    const pointCountOffset = config.xAxis.align === 'center' ? 0 : 1;
    // 如果 xAxis align center，则线的起始点为 halfUnitLength
    const unitLength = width / (values.pointCount - pointCountOffset);
    const xOffset = config.xAxis.align === 'center' ? (unitLength / 2) : 0;
    // 在 line view 中移动的距离
    const index = _.round((currentX - xOffset) / unitLength);
    ...
    const translateX = (index * unitLength) + yAxisWidth + xOffset;
  ...
  updateLines() {
    ...
    const pointCountOffset = config.xAxis.align === 'center' ? 0 : 1;
    // 如果 xAxis align center，则线的起始点为 halfUnitLength
    const unitLength = lineView.width / (values.pointCount - pointCountOffset);
    const xOffset = config.xAxis.align === 'center' ? (unitLength / 2) : 0;
    const arr = _.map(lineData, (item, index) => {
      ...
      return _.map(data, (v, i) => {
        ...
        return {
          x: (unitLength * i) + xOffset,
          y,
        };
      });
    });
    const curveFn = getFn(config.curve);
    const fn = d3.line()
      .x(d => d.x)
      .y(d => d.y)
      .curve(curveFn);
