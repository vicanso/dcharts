...
export default class Pip extends Chart {
  ...
  init(data) {
    ...
    const outerHeight = height || (radius * 2) + 100;
    target
    ...
    this.addLegend(data, outerWidth, outerHeight);
  ...
  addLegend(data, outerWidth, outerHeight) {
    const {
      chart,
      config,
    } = this;
    ...
    const x = (outerWidth - legend.view.width) / 2;
    const y = (outerHeight / 2) + radius + 30;
    labelContainer.attr('transform', `translate(${x}, ${y})`);
    return this;
  }
  render(data) {
