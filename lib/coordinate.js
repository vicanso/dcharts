...
export default class Coordinate extends Chart {
  ...
  getYAxisData() {
    ...
    const stepList = [];
    _.forEach([1, 10, 100, 1000, 10 * 1000, 100 * 1000, 1000 * 1000], (multiple) => {
      _.forEach([1, 2, 3, 4, 5, 6, 7, 8, 9], (step) => {
        stepList.push(step * multiple);
      });
    });
    const sortedIndex = _.sortedIndex(stepList, offset);
    const newOffset = stepList[sortedIndex] || offset;
    const arr = [];
    let start = range[0];
  ...
  getLineView() {
    ...
    const legendHeight = config.disabled.legend ? 0 : config.legend.height;
    return {
      x,
      y,
      width: view.width - x - margin.right,
      height: view.height - y - margin.bottom - config.xAxis.height - legendHeight,
    };
  }
  getRange(data) {
    const {
  ...
  initXAxis() {
    ...
    const lineView = this.getLineView();
    const legendHeight = config.disabled.legend ? 0 : config.legend.height;
    const translateY = view.height - xAxisHeight - config.margin.bottom - legendHeight;
    xAxis
      .set('horizontal', {
        align: this.get('xAxis.align', 'left'),
        height: xAxisHeight,
        width: lineView.width,
        distance: this.get('xAxis.distance', 30),
      });
