import * as _ from 'lodash';
import Chart from './chart';
import {
  setStyle,
} from './util';

export default class Axis extends Chart {
  constructor(target, config) {
    super(target, config);
    this.divideCount = 0;
    this.labelList = [];
    this.divisor = 0;
  }
  get name() {
    return 'axis';
  }
  getAxisLength() {
    const type = this.config.type;
    const {
      width,
      height,
    } = this.config[type];
    const length = type === 'vertical' ? height : width;
    return parseFloat(length) - 1;
  }
  getDivisor(max, refresh) {
    const {
      pointCount,
      type,
    } = this.config;
    if (!this.divisor || refresh) {
      const {
        distance,
      } = this.config[type];
      const axisLength = this.getAxisLength();
      const count = _.ceil(axisLength / distance);
      let divisor = 1;
      while ((count * divisor) + 1 < max) {
        divisor++;
      }
      this.divisor = divisor;
    }
    return this.divisor;
  }
  getDivideCount(length) {
    const {
      pointCount,
    } = this.config;
    if (!this.divideCount) {
      let max = length;
      const {
        distance,
      } = this.config[this.config.type];
      const axisLength = this.getAxisLength();
      let count = _.ceil(axisLength / distance);
      if (pointCount === 'auto') {
        if (count < max) {
          if (max % 2 !== 0) {
            max++;
          }
          while (count < max) {
            max >>= 1;
          }
        }
        count = max;
      }
      this.divideCount = count;
    }
    return this.divideCount;
  }
  getLineAxisPath(chart, max) {
    const {
      type,
    } = this.config;
    const {
      width,
      align,
    } = this.config[type];
    const axisLength = this.getAxisLength();
    const distanceOffset = Math.floor(axisLength / (max - 1));
    const offsetSize = type === 'vertical' ? width - 10 : 10;
    const divisor = this.getDivisor(max);
    const paths = [];
    for (var i = 0; i < max; i++) {
      let offset = i * distanceOffset;
      if (type === 'vertical') {

      } else {
        let y = offsetSize;
        let verticalPath = 'V0';
        if (i % divisor !== 0) {
          y = 0;
          verticalPath = '';
        }
        paths.push(`M${offset},${y}${verticalPath}H${distanceOffset}`);
      }
    }
    // const count = this.getDivideCount(max) - (align === 'center' ? 0 : 1);
    // const distanceOffset = axisLength / count;
    // let offset = 1;
    // const paths = [];
    // for (let i = 0; i <= count; i++) {
    //   if (type === 'vertical') {
    //     paths.push(`M${width - 10},${offset}H${width - 1}V${distanceOffset}`);
    //   } else {
    //     paths.push(`M${offset},${offsetSize}V0H${distanceOffset}`);
    //   }
    //   offset += distanceOffset;
    // }
    return paths.join('');
  }
  appendLineAxis(chart, max) {
    const style = this.config.style;
    const path = this.getLineAxisPath(chart, max);
    const baseLine = chart.append('path');
    baseLine.attr('d', path);
    setStyle(baseLine, style.line);
    return this;
  }
  appendTextAxis(data, chart) {
    return this.updateTextAxis(data, chart);
  }
  updateTextAxis(data, chart) {
    const {
      style,
      type,
    } = this.config;
    const {
      width,
      height,
      align,
    } = this.config[type];
    const max = data.length;
    const divisor = this.getDivisor(max);
    const axisLength = this.getAxisLength();
    const distance = Math.floor(axisLength / (max - 1));
    const labelList = this.labelList;
    let offset = 0;
    const key = 'y';
    const value = height * 0.6;
    let index = 0;
    _.forEach(data, (text, i) => {
      if (i % divisor !== 0) {
        return;
      }
      let label = labelList[index++];
      if (!label) {
        label = chart.append('text')
          .attr(key, value);
        labelList.push(label);
        setStyle(label, style.text);
      }
      label.text(text);
      offset = i * distance;
      const bBox = label.node().getBBox();
      if (type === 'vertical') {
        const y = offset + ((distance - bBox.height) / 2);
        label.attr('y', y + 10);
        label.attr('x', width - bBox.width - 5);
      } else {
        const haldBBoxWidth = bBox.width / 2;
        label.attr('x', offset - haldBBoxWidth);
      }
    });

    // for (var i = 0; i < max; i++) {
    //   let offset = i * distanceOffset;
    //   if (type === 'vertical') {

    //   } else {
    //     let y = offsetSize;
    //     let verticalPath = 'V0';
    //     if (i % divisor !== 0) {
    //       y = 0;
    //       verticalPath = '';
    //     }
    //     paths.push(`M${offset},${y}${verticalPath}H${distanceOffset}`);
    //   }
    // }

    // const count = this.getDivideCount(data.length);
    // const showIndex = parseInt(data.length / count, 10);
    // const distance = this.getAxisLength() / (count - (align === 'center' ? 0 : 1));
    // let offset = 0;
    // const key = 'y';
    // const value = height * 0.6;
    // const labelList = this.labelList;
    // let index = 0;
    // _.forEach(data, (text, i) => {
    //   if (i % showIndex !== 0) {
    //     return;
    //   }
    //   let label = labelList[index++];
      // if (chart) {
      //   label = chart.append('text')
      //     .attr(key, value);
      //   labelList.push(label);
      //   setStyle(label, style.text);
      // }
      // label.text(text);

      // const bBox = label.node().getBBox();
      // if (type === 'vertical') {
      //   const y = offset + ((distance - bBox.height) / 2);
      //   label.attr('y', y + 10);
      //   label.attr('x', width - bBox.width - 5);
      // } else {
      //   let x = 0;
      //   const haldBBoxWidth = bBox.width / 2;
      //   if (align === 'left') {
      //     x = (distance * (index - 1)) - haldBBoxWidth;
      //   } else {
      //     x = offset + ((distance - bBox.width) / 2);
      //   }
      //   label.attr('x', x);
      // }
      // offset += distance;
    // });
    return this;
  }
  init(data = []) {
    if (this.initialized) {
      return this;
    }
    const { target } = this;
    const {
      width,
      height,
    } = this.config[this.config.type];

    target.attr('width', width)
      .attr('height', height);
    this.appendLineAxis(target.append('g'), data.length);
    this.appendTextAxis(data, target.append('g'));
    this.initialized = true;
    return this;
  }
  render(data) {
    return this.init(data || []);
  }
  update(data = []) {
    this.updateTextAxis(data);
  }
}
