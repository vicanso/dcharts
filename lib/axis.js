...
export default class Axis extends Chart {
  ...
  getLineAxisPath(chart, count) {
    ...
    const axisLength = this.getAxisLength();
    const distanceOffset = axisLength / (count - offset);
    const offsetSize = type === 'vertical' ? width - 10 : 10;
    const divisor = this.getDivisor(count);
    const max = align === 'left' ? count : count + 1;
    const paths = [];
    for (let i = 0; i < max; i += 1) {
      const moveOffset = Math.min(i * distanceOffset, axisLength);
      if (i % divisor === 0) {
        this.divideCount += 1;
      }
      if (type === 'vertical') {
        ...
        let verticalPath = 'V0';
        let moveToPath = '';
        const h = Math.min(moveOffset + distanceOffset, axisLength);
        if (i % divisor !== 0) {
          y = 0;
          verticalPath = '';
        } else {
          moveToPath = `M${moveOffset},${y}`;
        }
  ...
  updateTextAxis(chart, data) {
    ...
    _.forEach(data, (text, i) => {
      ...
      label.text(fortmatNumber(text, 2));
      ...
      if (type === 'vertical') {
        ...
        if (bBox.width > distance && index % 2 === 0) {
          label.remove();
          return;
        }
